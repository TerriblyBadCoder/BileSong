...
public class ParryParticle<T extends ParryParticleEffect> extends SpriteBillboardParticle {
    private final Vec3d direction;
    private float yaw;
    private SpriteProvider provider;
    private float pitch;
    protected ParryParticle(ClientWorld clientWorld, double d, double e, double f, SpriteProvider spriteProvider, double xd, double yd, double zd, ParryParticleEffect effect, SpriteProvider provider) {
        ...
        if(effect.getHeat()!=0){
            this.blue = (effect.getHeat() & (((1 << (24-16)) - 1) << 16 ))/255.0f;
            this.green = (effect.getHeat() & (((1 << (16-8)) - 1) << 8 ))/255.0f;
            this.red = (effect.getHeat() & (((1 << (8-0)) - 1) << 0 ))/255.0f;
        }
    ...
    public static class Factory implements ParticleFactory<ParryParticleEffect> {
        ...
        public Particle createParticle(ParryParticleEffect particleType, ClientWorld level, double x, double y, double z,
            ...
            return new ParryParticle<>(level, x, y, z, this.sprites, dx, dy, dz,particleType,sprites);
        }
    }
}
